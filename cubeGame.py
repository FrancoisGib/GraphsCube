...
class Plateau:
    ...
    def __eq__(self, __o: object) -> bool:
        ...
        if not isinstance(__o, Plateau): # si __o n'est pas un plateau, ils ne sont pas égaux
            return NotImplemented
        return np.array_equal(self.terrain, __o.terrain) and self.cube == __o.cube   and __o.lig == self.lig and __o.col == self.col
    ...
    def _update_face(self) -> None:
        """les valeurs de la face du dé et de la case de terrain correspondante
           sont échangées
        """
        self.cube.vals[FACE], self.terrain[(self.col, self.lig)] = self.terrain[(self.col, self.lig)], self.cube.vals[FACE]
    ...
    def haut(self) -> Plateau:
        ...
        if self.lig > 0:
            plateauCopy = self.copy()
            plateauCopy.cube.haut()
            plateauCopy.lig -= 1
            plateauCopy._update_face()
            return plateauCopy
        raise HorsPlateau()
    ...
    def final(self) -> bool:
        ...
        return np.array_equal(self.terrain, np.full((4, 4), False)) # gagné si toutes les cases du plateau sont à False
